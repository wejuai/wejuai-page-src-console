...
export default {
  ...
  methods: {
    list(page) {
      ...
      service.get(api.feedback + "?start=" + start + "&end=" + end + "&page=" + page + "&size=" + this.query.size
    ...
    handle(row) {
      if (StringUtils.isBlank(row.userId)) {
        service.put(api.feedback + "/" + row.id).then(res => {
        ...
        this.$prompt('请输入处理意见', '提示', {
          ...
          service.put(api.feedback + "/" + row.id + "?content=" + value).then(res => {
            this.$message.success("处理成功");
            this.list(this.query.page);
          })
        }).catch(() => {
